...
def create_playlist(sp, username, sorted_tracks, playlist_id, playlist):
    ...
    for sorted_track in sorted_tracks:
        track_ids.append(sorted_track['track']['id'])
    results = sp.user_playlist_create(user=username, name="Sortified " + playlist['name'], public=True,
...
def sort_by_key(items, key):
    return {
        ...
        'Alphabetical Z-A': sorted(items, key=lambda x: x['track']['name'], reverse=True),
        'Artist': sorted(items, key=lambda x: clean_artist(x['track']['artists'][0]['name'])),
        'Chronological': sorted(items, key=lambda k: k['track']['album']['release_date'], reverse=True),
        'Non-Chronological': sorted(items, key=lambda k: k['track']['album']['release_date'], reverse=False),
        'Added At': sorted(items, key=lambda k: datetime.strptime(k["added_at"], "%Y-%m-%dT%H:%M:%SZ"), reverse=True)
    }[key]
